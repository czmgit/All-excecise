...
#卷积核
class Filter(object):
    def __init__(self,filter_height,filter_width,depth):
        self.weights = np.random.uniform(-1e-4,-1e-4,(depth,filter_height,filter_width))
        self.bias = 0
        #权重梯度？
...
def element_wise_op(array,op):
    for i in np.nditer(array,op_flags = ['readwrite']):
        print(i)
        #用省略号代替列表切片中的冒号,通过将op函数返回的值赋值给i来达到改变数组array的目的，即改变了i就相当于改变了array
        i[...] = op(i)

def get_patch(input_array,i,j,width,height,stride):
    #从输入数组中获取本次卷积的区域，自动适配输入为2D和3D的情况.本函数获取的是卷积区域
    start_i = i*stride
    start_j = j*stride
    if input_array.ndim == 2:
        return input_array[start_i:start_i+height,start_j:start_j+width]
...
def padding(input_array,zp):
    #为数组增加zero_padding,自动适配输入为2D和3D的情况
    if zp ==0:
        return input_array
    else:
...
class ConvLayer(object):
    def __init__(self,input_width,input_height,channel_number,filter_width,
        ...
        # 卷积步长
        self.stride = stride
        self.output_width = int(ConvLayer.calculate_output_size(input_width,filter_width,zero_padding,stride))
        self.output_height = int(ConvLayer.calculate_output_size(input_height,filter_height,zero_padding,stride))
        self.output_array = np.zeros((self.filter_number,self.output_height,self.filter_width))
        self.filters = []
        for i in range(filter_number):
            self.filters.append(Filter(filter_height,filter_width,self.channel_number))
        self.activator = activator
        self.learning_rate = learning_rate

    def forward(self,input_array):
        #计算卷积层的输出，输出结果存在self.ouput_array
        self.input_array = input_array
        self.padding_input_array = padding(input_array,self.zero_padding)
        for f in range(self.filter_number):
            filter = self.filters[f]
            conv(self.padding_input_array,filter.get_weights(),
    ...
    def bp_sensitivity_map(self,sensitivity_array,activator):
        #计算传递到上一层的sentivity_array map
        #sensitivity_map:本层的sensitivity map
        #activator:上一层的激活函数
        ...
        for f in range(self.filter_number):
            ...
            # sensitivity map相当于所有的filter的sensitivity map之和，为什么多个卷积核的误差项要加到一起？
            #
        ...
        self.delta_array *= derivative_array#这里相乘是什么意思？
    ...
    def bp_gradient(self,sensitivity_array):
        ...
        for f in range(self.filter_number):
            ...
            for d in range(filter.weights.shape[0]):
                conv(self.padding_input_array[d],expanded_array[f],filter.weights_grad[1],1,0)
            #计算偏置项的梯度
            filter.bias_grad = expanded_array[f].sum()

    #这里的sensitivity_array就是误差项
    def expand_sensitivity_map(self,sensitivity_array):
        depth = sensitivity_array.shape[0]
        #确定扩展后sensitivity map 的大小，计算stride为1时map的大小
    ...
    @staticmethod
    def calculate_output_size(input_size, filter_size, zero_padding, stride1):
        return (input_size - filter_size + 2 * zero_padding) / stride1 + 1
...
class MaxPoolingLayer(object):
    ...
    def forward(self,input_array):
        for d in range(self.channel_number):
            for i in range(self.output_height):
                for j in range(self.output_width):
                    self.output_array[d,i,j] = (get_patch(input_array[d],i,j,self.filter_width,
        ...
        for d in range(self.channel_number):
            for i in range(self.output_height):
                for j in range(self.output_width):
                    ...
                    self.delta_array[d,i*self.stride+k,j*self.stride+1] = sensitivity_array[d,i,j]
